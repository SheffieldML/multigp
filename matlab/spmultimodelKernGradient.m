function [gKernParam, gX_u] = spmultimodelKernGradient(model,dLdKyy, dLdKuy, dLdKuu)

% SPMULTIMODELKERNGRADIENT Gradients of the parameters for spmultimodel.
%
% FORMAT
...
if strcmp(model.kernType, 'lmc')
    ...
    for c = 1:length(model.kern.comp)
        endVal3 = endVal3 + model.kern.comp{c}.nParams;
        gMulti = zeros(1, model.kern.comp{c}.nParams);
        startVal = 1;
        endVal = 0;
        for i = 1:length(model.kern.comp{c}.comp)
            endVal = endVal + model.kern.comp{c}.comp{i}.nParams;
            if i<=model.nlf
            ...
            if ~isempty(covPartial)                
                if i == 1
                    localGradFunc = model.kernFuncNames.gradientLat;
                    localGradVal = real(localGradFunc(model.kern.comp{c}.comp{i}, model.X{i}, covPartial));
                    localParamsValLat = model.kernFuncNames.extractLat(model.kern.comp{c}.comp{i});
                    gMulti(1, startVal:endVal) = localGradVal(model.kernFuncNames.transformLat.index).*...
                        localParamsValLat(model.kernFuncNames.transformLat.index);
                        
                else
                    localGradFunc = model.kernFuncNames.gradientOut;
                    localGradVal = real(localGradFunc(model.kern.comp{c}.comp{i}, model.X{i}, ...
                        covPartial));
                    localParamsValOut = model.kernFuncNames.extractOut(model.kern.comp{c}.comp{i});
                    gMulti(1, startVal:endVal) = localGradVal(model.kernFuncNames.transformOut.index).*...
                        localParamsValOut(model.kernFuncNames.transformOut.index);
                end
            ...
            if i > model.nlf && c <= model.nlf
                ...
                [g1, g2] = model.kernFuncNames.gradientCross(model.kern.comp{c}.comp{i}, ...
                    model.kern.comp{c}.comp{1}, model.X{i}, model.X{c}, dLdKuy{c,i - model.nlf}');
                g1  = g1(model.kernFuncNames.transformOut.index).*...
                    localParamsValOut(model.kernFuncNames.transformOut.index);
                g2 = g2(model.kernFuncNames.transformLat.index).*...
                    localParamsValLat(model.kernFuncNames.transformLat.index);
