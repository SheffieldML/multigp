function [Kyy, Kyu, Kuu] = simglobalKernCompute(kern, outX, latX, gamma)
...
for i = 1:kern.nout,
    ...
    for j = 1: kern.nlf
        ...
        % Compute Kff        
        Kyy{i,j} = real(kern.funcNames.computeOut(kernOut, outX{i}));
        % Compute Kfu, which corresponds to K_{\hat{f}}u, really.
        Kyu{i,j} = real(kern.funcNames.computeCross(kernOut, kernLat, outX{i}, latX{j}));
    end
end
