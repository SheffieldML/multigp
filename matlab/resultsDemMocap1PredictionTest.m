function model = resultsDemMocap1PredictionTest(dirResults, ...
    experimentName, subject, numberChannels, numberMissing, ...
    numberLatentFunctions, experiment, useInd);

% RESULTSDEMMOCAP1PREDICTIONTEST description.
...
% Xt = linspace(min(Xtest{1})-0.2,max(Xtest{1})+0.2,300)';
Xt = Xtest{1};
[mu, varsigma] = multigpPredictionMeanVarTest(model, Xt, numberMissing, options);
close all
